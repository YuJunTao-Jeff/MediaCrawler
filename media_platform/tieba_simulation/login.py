...
class TiebaSimulationLogin(AbstractLogin):
    """贴吧模拟登录"""
    ...
    async def begin(self) -> None:
        """开始登录流程"""
        utils.logger.info("[TiebaSimulationLogin] 开始模拟登录流程")
        ...
        try:
            ...
            utils.logger.error(f"[TiebaSimulationLogin] 登录失败: {e}")
            raise e
    
    async def _qrcode_login(self) -> None:
        """二维码登录"""
        utils.logger.info("[TiebaSimulationLogin] 使用二维码登录")
        ...
        try:
            ...
            for selector in login_selectors:
                try:
                    ...
                    if element and await element.is_visible():
                        utils.logger.info(f"[TiebaSimulationLogin] 尝试点击登录元素: {selector}")
                        await element.click(force=True)
                        login_success = True
                        break
                except Exception as e:
                    utils.logger.debug(f"[TiebaSimulationLogin] 点击失败 {selector}: {e}")
                    continue
            
            # 方法2: 如果直接点击失败，尝试键盘导航
            if not login_success:
                utils.logger.info("[TiebaSimulationLogin] 尝试键盘导航到登录按钮")
            ...
            for qr_selector in qr_selectors:
                try:
                    ...
                    qr_found = True
                    utils.logger.info(f"[TiebaSimulationLogin] 找到二维码: {qr_selector}")
                    break
                except:
                    continue
            
            if not qr_found:
                utils.logger.warning("[TiebaSimulationLogin] 未找到二维码，但继续等待登录")
            
            utils.logger.info("[TiebaSimulationLogin] 请使用百度APP扫描二维码登录，或手动完成登录流程")
    ...
    async def _phone_login(self) -> None:
        """手机号登录"""
        utils.logger.info("[TiebaSimulationLogin] 使用手机号登录")
        ...
        try:
            # 点击登录按钮
            login_button_selector = ".u_login, .login-btn"
            ...
            utils.logger.info("[TiebaSimulationLogin] 请输入手机验证码")
    ...
    async def _cookie_login(self) -> None:
        """Cookie登录"""
        utils.logger.info("[TiebaSimulationLogin] 使用Cookie登录")
        ...
        try:
            ...
            if await self._check_login_status():
                utils.logger.info("[TiebaSimulationLogin] Cookie登录成功")
            else:
    ...
    async def _wait_for_login_success(self, timeout: int = 300000) -> None:
        """等待登录成功"""
        utils.logger.info("[TiebaSimulationLogin] 等待登录成功...")
        ...
        while True:
            ...
            if await self._check_login_status():
                utils.logger.info("[TiebaSimulationLogin] 登录成功！")
                break
        ...
        try:
                ...
                try:
                    ...
                    for element in elements:
                        if await element.is_visible():
                            utils.logger.info(f"[TiebaSimulationLogin] 发现遮罩层，尝试点击: {selector}")
                            await element.click(force=True)
                            await asyncio.sleep(0.5)
                except Exception as e:
                    utils.logger.debug(f"[TiebaSimulationLogin] 处理遮罩失败 {selector}: {e}")
                    continue
            ...
            for selector in close_selectors:
                try:
                    element = await self.context_page.query_selector(selector)
                    if element and await element.is_visible():
                        utils.logger.info(f"[TiebaSimulationLogin] 发现关闭按钮，尝试点击: {selector}")
                        await element.click()
                        await asyncio.sleep(0.5)
                except Exception as e:
                    utils.logger.debug(f"[TiebaSimulationLogin] 处理关闭按钮失败 {selector}: {e}")
                    continue
            ...
            utils.logger.warning(f"[TiebaSimulationLogin] 处理页面遮罩失败: {e}")