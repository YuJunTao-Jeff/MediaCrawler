...
class ContentItem:
    ...
    def from_model(cls, model_instance, platform: str):
        ...
        return cls(
            ...
            publish_time=datetime.fromtimestamp(getattr(model_instance, field_mapping['publish_time']) / 1000),
...
def get_interaction_count(model_instance, platform: str) -> int:
    ...
    try:
        if platform == 'xhs':
            ...
            comment = int(getattr(model_instance, 'comment_count', '0') or '0')
            return liked + comment + viewed // 100  # 播放量按百分之一计算
            ...
            comment = int(getattr(model_instance, 'comment_count', '0') or '0')
            praise = int(getattr(model_instance, 'praise_count', '0') or '0')
